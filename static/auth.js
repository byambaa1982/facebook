...
function hideAuthModal() {
    ...
    if (modal) {
        console.log('Hiding auth modal - current display:', modal.style.display);
        modal.style.display = 'none';
        modal.style.visibility = 'hidden';
        modal.style.opacity = '0';
        modal.style.zIndex = '-1';
        console.log('Modal hidden - new display:', modal.style.display);
    } else {
...
async function handleLogin(event) {
    ...
    try {
        ...
        if (data.success) {
            ...
            console.log('Setting timeout to hide modal and refresh auth');
            setTimeout(() => {
                console.log('Timeout executing - hiding modal and refreshing auth');
                hideAuthModal();
...
async function logout() {
    ...
    try {
        ...
        if (data.success) {
            console.log('Logout successful, refreshing auth status');
            checkAuthStatus(); // Refresh auth status
        } else {
